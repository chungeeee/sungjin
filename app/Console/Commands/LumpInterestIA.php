...
class LumpInterestIA extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'LumpInterest:InterestIA {batchNo?}';
     ...
     * @var string
     */
    protected $description = '이자지급내역(국민은행)';
    ...
    public function handle()
    {
        ...
        // 메인쿼리
        $result = DB::TABLE("lump_master_log")->SELECT("*")->WHERE('division','IA')
                                                         ->WHERE('status','W')
                                                         ->ORDERBY('no', 'asc')
                                                         ->first();
        ...
        if(!empty($result))
        {
            ...
            if(Storage::disk('public')->exists($path))
            {
                ...
                $colHeader  = array('은행코드','계좌번호','금액','이름','받는사람메모','보내는사람메모');
                $colNm      = array(
                    'bank_cd'	            => '0',	    // 은행코드
                    'bank_ssn'	            => '1',	    // 계좌번호
                    'interest_money'	    => '2',	    // 금액
                    'bank_owner'            => '3',     // 이름
                    'receiver_memo'         => '4',     // 받는사람메모
                    'sender_memo'           => '5'      // 보내는사람메모
                );
                ...
                {
                    // 엑셀파일 헤더 불일치
                    $rslt = DB::dataProcess("UPD", "lump_master_log", ['remark'=>'엑셀파일 헤더 불일치 오류','status'=>'X',], ['division'=>'IA', 'no'=>$result->no]);
                }
                else
                {
                    // 상태 '진행중' 변경
                    $rslt = DB::dataProcess("UPD", "lump_master_log", ['status'=>'P','start_time'=>time()], ['division'=>'IA', 'no'=>$result->no]);
                    ...
                    $rslt = DB::dataProcess('UPD', 'lump_master_log', ["status"=>"C","finish_time"=>time(),"total_count"=>$total_cnt, "ok_count"=>(($total_cnt)-count($ERROR)), "fail_count"=>count($ERROR), "remark"=>count($ERROR)."건 실패"], ['division'=>'IA', "no"=>$result->no]);
                }
            }
            else
            {
                $rslt = DB::dataProcess("UPD", "lump_master_log", ['remark'=>'엑셀파일 읽기오류','status'=>'X',], ['division'=>'IA', 'no'=>$result->no]);
                echo "엑셀파일 미존재\n";
            }
        }
        else
        {
            echo "대기중인 이자지급내역(국민) 배치 미존재\n";
        }
        ...
        {      
            ...
            $rslt = DB::dataProcess("UPD", "lump_master_log", ['fail_file_path'=>$file_path,'fail_file_name'=>$file_name], ['division'=>'IA', 'no'=>$result->no]);
	    }
