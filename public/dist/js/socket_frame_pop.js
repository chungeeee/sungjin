...
function parseMessage(msg)
{
	displayText("R", msg);
		...
		parent.parseMemberStatus(kind);
                ...
                var isEvent = false;
                // 설정값 확인 
                var in_chk = getCallset('in_chk');
                var inbound = getCallset('inbound');
                var out_chk = getCallset('out_chk');
                var outbound = getCallset('outbound');
                console.log(in_chk + '|' + inbound + '|' + out_chk + '|' + outbound);
                // 전화이벤트
                if(kind=='IR' && in_chk=='Y' && inbound=='A1')
                {
                    kind = 'IR_POP';
                    isEvent = true;
                }
                else if(kind=='ID' && in_chk=='Y' && inbound=='A2')
                {
                    kind = 'ID_POP';
                    isEvent = true;
                }
                else if(kind=='OR' && out_chk=='Y' && outbound=='B1')
                {
                    kind = 'OR_POP';
                    isEvent = true;
                }
                else if(kind=='OD' && out_chk=='Y' && outbound=='B2')
                {
                    kind = 'OD_POP';
                    isEvent = true;
                }
