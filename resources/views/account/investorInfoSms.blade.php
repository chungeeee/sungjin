...
<script>
    ...
    function imgAction(mode)
    {
        ...
        $.ajax({
            url  : "/account/investorinfosmsaction",
            type : "post",
            data : postdata,
            processData : false,
            contentType : false,
            success : function(result) {
                globalCheck = false;
                alert(result);
                getInvestorData('investorinfosms');
            },
            error : function(xhr) {
                globalCheck = false;
...
<b>문자</b>
<!-- BODY -->
{{-- <button type="button" class="btn btn-xs btn-outline-info float-right mb-1" onclick="getInvestorData('investorinfosms');"><i class="fa fa-plus-square text-info mr-1"></i>파일추가</button> --}}
<table class="table table-sm table-hover loan-info-table card-secondary card-outline">
    ...
    <tbody>
        @forelse( $img as $idx => $v )
            <tr onclick="getInvestorData('investorinfosms','',{{ $v->no }});" @if( isset($selected_img[0]->no) && $selected_img[0]->no == $v->no ) bgcolor="FFDDDD" @endif >
                ...
                @endif
                <td class="text-center">{{ $v->messages ?? '' }}</td>
                <td class="text-center">{{ $v->save_id }}</td>
                <td class="text-center">{{ $v->save_time }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="13" class='text-center p-4'>등록된 메세지가 없습니다.</td>
            </tr>
        @endforelse
