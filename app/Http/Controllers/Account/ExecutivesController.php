...
class ExecutivesController extends Controller
{
    ...
    /**
     * 임원목록 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataExecutivesList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"executives","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/executivesexcel', 'form_executives')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataExecutivesList($request);
        ...
        return view('account.executives')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function executivesList(Request $request)
    {
        $list  = $this->setDataExecutivesList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function executivesForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.executivesForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function executivesAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function executivesExcel(Request $request)
    {
        // 실행시간 제어
