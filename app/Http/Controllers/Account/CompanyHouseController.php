...
class CompanyHouseController extends Controller
{
    /**
    ...
    /**
     * 사택관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataCompanyHouseList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"companyHouse","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/companyhouseexcel', 'form_companyHouse')", "btn-success");
        }
     ...
     * @return view
     */
    public function companyHouse(Request $request)
    {
        $list   = $this->setDataCompanyHouseList($request);
        ...
        return view('account.companyHouse')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function companyHouseList(Request $request)
    {
        $list  = $this->setDataCompanyHouseList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function companyHouseForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.companyHouseForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function companyHouseAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function companyHouseExcel(Request $request)
    {
        // 실행시간 제어
