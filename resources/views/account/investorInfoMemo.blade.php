...
<script>
    ...
    function imgAction(mode)
    {
        ...
        $.ajax({
            url  : "/account/investorinfomemoaction",
            type : "post",
            data : postdata,
            processData : false,
            contentType : false,
            success : function(result) {
                globalCheck = false;
                alert(result);
                getInvestorData('investorinfomemo');
            },
            error : function(xhr) {
                globalCheck = false;
...
<b>메모</b>
<!-- BODY -->
{{-- <button type="button" class="btn btn-xs btn-outline-info float-right mb-1" onclick="getInvestorData('investorinfomemo');"><i class="fa fa-plus-square text-info mr-1"></i>파일추가</button> --}}
<table class="table table-sm table-hover loan-info-table card-secondary card-outline">
    ...
    <tbody>
        @forelse( $img as $idx => $v )
            <tr onclick="getInvestorData('investorinfomemo','',{{ $v->no }});" @if( isset($selected_img[0]->no) && $selected_img[0]->no == $v->no ) bgcolor="FFDDDD" @endif >
