...
class LineFormatter extends NormalizerFormatter
{
    ...
    public function format(LogRecord $record): string
    {
        ...
        if (false !== strpos($output, '%ip%')) {
            $ip = ( isset($_SERVER) && array_key_exists('REMOTE_ADDR', $_SERVER) ) ? $_SERVER['REMOTE_ADDR'] : "0.0.0.0";
            $output = str_replace('%ip%', $ip, $output);
        }
        if (false !== strpos($output, '%id%')) {
            $id = ( Auth::check() ) ? Auth::user()->id : "Anonymous" ;
            $output = str_replace('%id%', $id, $output);
        }
        if (false !== strpos($output, '%method%')) {
            $method = request()->method();
            $output = str_replace('%method%', $method, $output);
        }
