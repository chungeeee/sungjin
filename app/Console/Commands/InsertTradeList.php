...
class InsertTradeList extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Insert:tradeList {batchNo?}';
     ...
     * @var string
     */
    protected $description = '우리은행 입금 등록';
    ...
    public function handle(): void
    {
        ...
        {
            $myTable = 'WCMS_GW_ACCOUNT_TRNX_LOG'; 
        }
        else 
        {
            $myTable = 'TEST_WCMS_GW_ACCOUNT_TRNX_LOG';
        }

        // 안가져온것 같져오기
        $wcms = DB::connection('mybank')->table($myTable)
                    ->select("*")
                    ->whereRaw(" COALESCE(TRADE_LIST_FLAG, '')!='Y' ")
                    ->whereNotIn('TRNX_TYPE', ['23', '28', '29'])
                    // ->whereIn('TR_NO', ['000001','000002','000003','000004'])
        ...
        foreach( $wcms as $v )
        {
            ...
            $result = DB::dataProcess('INS', 'trade_list', $IN);
                        
            // 성공
            if($result=='Y')
            {
                // 업데이트
                $upResult = DB::connection('mybank')->table($myTable)
                ->whereRaw(" COALESCE(TRADE_LIST_FLAG, '')!='Y' ")
                ->where('TRNX_DATE', $IN['in_date'])
                ->where('TR_NO', $IN['str_no'])
                ->where('ACCOUNT_NO', $IN['mo_ssn'])
                ->update(['TRADE_LIST_FLAG' => "Y"]);
                ...
                else 
                {
                    ...
                    // 실패로그 남긴다.
                    $note = $IN['in_date']." ".$IN['str_no'];
                    echo $note."\n";
                    Log::debug($myTable.' 입력 에러');
                    Log::debug($note);
                    DB::rollback();
                }
            }
