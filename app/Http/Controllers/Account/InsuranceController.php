...
class InsuranceController extends Controller
{
    ...
    /**
     * 보험관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataInsuranceList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"insurance","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/insuranceexcel', 'form_insurance')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataInsuranceList($request);
        ...
        return view('account.insurance')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function insuranceForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.insuranceForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function insuranceExcel(Request $request)
    {
        // 실행시간 제어
