...
class CardController extends Controller
{
    ...
    /**
     * 카드거래관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataCardList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"card","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/cardexcel', 'form_card')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataCardList($request);
        ...
        return view('account.card')->with('result', $list->getList());
    }   
    ...
    public function cardForm(Request $request)
    {
        ...
        return view('account.cardForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function cardExcel(Request $request)
    {
        // 실행시간 제어
