...
class CorporateCarController extends Controller
{
    /**
    ...
    /**
     * 법인차관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataCorporateCarList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"corporateCar","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/corporatecarexcel', 'form_corporateCar')", "btn-success");
        }
     ...
     * @return view
     */
    public function corporateCar(Request $request)
    {
        $list   = $this->setDataCorporateCarList($request);
        ...
        return view('account.corporateCar')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function corporateCarList(Request $request)
    {
        $list  = $this->setDataCorporateCarList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function corporateCarForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.corporateCarForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function corporateCarAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function corporateCarExcel(Request $request)
    {
        // 실행시간 제어
