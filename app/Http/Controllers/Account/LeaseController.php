...
class LeaseController extends Controller
{
    ...
    /**
     * 임대차관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataLeaseList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"lease","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/leaseexcel', 'form_lease')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataLeaseList($request);
        ...
        return view('account.lease')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function leaseForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.leaseForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function leaseAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function leaseExcel(Request $request)
    {
        // 실행시간 제어
