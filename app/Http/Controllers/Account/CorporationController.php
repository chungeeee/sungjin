...
class CorporationController extends Controller
{
    ...
    /**
     * 법인정보 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataCorporationList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"corporation","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/corporationexcel', 'form_corporation')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataCorporationList($request);
        ...
        return view('account.corporation')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function corporationList(Request $request)
    {
        $list  = $this->setDataCorporationList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function corporationForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.corporationForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function corporationAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function corporationExcel(Request $request)
    {
        // 실행시간 제어
