...
class ProductController extends Controller
{
    ...
    /**
     * 소모품관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataProductList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"product","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/productexcel', 'form_product')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataProductList($request);
        ...
        return view('account.product')->with('result', $list->getList());
    }   
    ...
    public function productForm(Request $request)
    {
        ...
        return view('account.productForm')->with("arrayConfig", $arrayConfig)
