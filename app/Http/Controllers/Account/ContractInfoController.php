...
class ContractInfoController extends Controller
{
    ...
    /**
     * 계약정보관리 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataContractInfoList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"contractInfo","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/contractinfoexcel', 'form_contractInfo')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataContractInfoList($request);
        ...
        return view('account.contractInfo')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function contractInfoList(Request $request)
    {
        $list  = $this->setDataContractInfoList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function contractInfoForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.contractInfoForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function contractInfoAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function contractInfoExcel(Request $request)
    {
        // 실행시간 제어
