...
<form class="form-horizontal" name="sanggak_cancel_form" id="sanggak_cancel_form">
<input type="hidden" name="loan_sanggak_seq" value="{{ $loan_sanggak_seq }}">

<div class="card card-lightblue">

    <div class="card-header">
    <h2 class="card-title">상각취소결재</h2>
    </div>
    ...
    <div class="card-body mr-3 p-3">
        ...
        <div class="form-group row">
            <label for="sg_cancel_reason_cd" class="col-sm-2 col-form-label">상각취소사유</label>
            <div class="col-sm-4">
                <select class="form-control form-control-sm" name="sg_cancel_reason_cd" id="sg_cancel_reason_cd">
                {{ Func::printOption($array_config['sg_cancel_reason_cd'], $rslt['sg_cancel_reason_cd']) }}
                </select>
            </div>
        ...
        <div class="form-group row mt-2">
            ...
            <div class="col-sm-4 col-form-label">
                {{ Func::getArrayName($array_user_id,$rslt['cancel_app_id']) }}
                ( {{ Func::dateFormat($rslt['cancel_app_time']) }} )
            </div>
        </div>
        <input type="hidden" name="cancel_app_id" value="{{ $rslt['cancel_app_id'] ?? '' }}" >
        @endif
        ...
        @if( $rslt['cancel_status']=="Y" )
        <div class="form-group row mt-2">
            <label for="search_string" class="col-sm-2 col-form-label">결재 일시</label>
            <div class="col-sm-4 col-form-label">
                {{  Func::getArrayName($array_user_id,$rslt['cancel_confirm_id']) }}
                ( {{ Func::dateFormat($rslt['cancel_confirm_time']) }} )
            </div>
        </div>
        @elseif( $rslt['cancel_status']=="N" )
        <div class="form-group row mt-2">
    ...
    <div class="card-footer">
        ...
        <button type="button" class="btn btn-sm btn-info   float-right mr-1" id="btn_confirm" onclick="sanggakCancelAction('CONFIRM');">결재</button>
        @endif
        <button type="button" class="btn btn-sm btn-danger float-right mr-1" id="btn_delete"  onclick="sanggakCancelAction('DELETE');" >취소</button>
        <button type="button" class="btn btn-sm btn-info float-right mr-1" id="btn_update"  onclick="sanggakCancelAction('UPDATE');" >수정</button>
        @elseif( $action_mode=="INSERT" )
        <button type="button" class="btn btn-sm btn-info float-right mr-3" id="cate_btn" onclick="sanggakCancelAction('INSERT');">상각취소요청 등록</button>
        @endif
    </div>
...
function sanggakCancelAction(md)
{
    if(!$("#loan_info_no").val())
    {
        alert("선택된 계약이 없습니다.");
    ...
    if( md=="INSERT" && !confirm("상각취소요청을 등록하시겠습니까?") )
    {
        return false;
    }
    if( md=="DELETE" && !confirm("상각취소요청을 취소하시겠습니까?") )
    {
        return false;
    }
    if( md=="CONFIRM" && !confirm("상각취소요청을 결재하시겠습니까?") )
    {
        return false;
    }
    ...
    var formData = new FormData($('#sanggak_cancel_form')[0]);
    ...
    $.ajax({
        url  : "/erp/sanggakcancelformaction",
        type : "post",
        data : formData,
        processData: false,
        contentType: false,
