...
class ShareholderController extends Controller
{
     ...
     * @return dataList
     */
    private function setDataShareholderList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"shareholder","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/shareholderexcel', 'form_shareholder')", "btn-success");
        }
    ...
    {
        $list   = $this->setDataShareholderList($request);
        ...
        return view('account.shareholder')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function shareholderList(Request $request)
    {
        $list  = $this->setDataShareholderList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function shareholderForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.shareholderForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function shareholderExcel(Request $request)
    {
        // 실행시간 제어
