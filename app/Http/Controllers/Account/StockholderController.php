...
class StockholderController extends Controller
{
    ...
    /**
     * 주주목록 조회 리스트 공통 세팅 내용 
     *
     * @param  request
     * @return dataList
     */
    private function setDataStockholderList(Request $request)
    {
        $list   = new DataList(Array("listName"=>"stockholder","listAction"=>'/'.$request->path()));

        if( Func::funcCheckPermit("C022") )
        {
            //$list->setButtonArray("엑셀다운", "excelDownModal('/account/stockholderexcel', 'form_stockholder')", "btn-success");
        }
     ...
     * @return view
     */
    public function stockholder(Request $request)
    {
        $list   = $this->setDataStockholderList($request);
        ...
        return view('account.stockholder')->with('result', $list->getList());
    }   
     ...
     * @return view
     */
    public function stockholderList(Request $request)
    {
        $list  = $this->setDataStockholderList($request);
        $param  = $request->all();
     ...
     * @return view
     */
    public function stockholderForm(Request $request)
    {
        $arrayConfig = Func::getConfigArr();
        $arrayBranch = Func::myPermitBranch();

        return view('account.stockholderForm')->with("arrayConfig", $arrayConfig)
     ...
     * @return view
     */
    public function stockholderAction(Request $request)
    {
        $v = $request->input();
     ...
     * @return view
     */
    public function stockholderExcel(Request $request)
    {
        // 실행시간 제어
